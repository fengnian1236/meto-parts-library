...
// ================================================================
// 3. Stage card button - use regex
// ================================================================
// Pattern: onclick="event.stopPropagation();editStage(\'' + curCategory + '\',\'' + (curHookId||'') + '\',\'' + s.cls + '\')" title="编辑此项">✎</button>
// Replace with: onclick="event.stopPropagation();openEditPartModal(\'' + curCategory + '\',\'' + (curHookId||'') + '\')" title="编辑此钩子">✎</button>

// The tricky part is matching the exact escaped quotes in the file.
// From debug5 we know the file has: editStage(\\'' + curCategory + '\\',\\'' + (curHookId||'') + '\\',\\'' + s.cls + '\\')
// This means: editStage(\'' + curCategory + '\',\'' + (curHookId||'') + '\',\'' + s.cls + '\')
// In regex, we need to match this literally.

let stageCount = 0;
content = content.replace(
    /onclick="event\.stopPropagation\(\);editStage\(\\'\\'' \+ curCategory \+ '\\'\\'',\\'\\'' \+ \(curHookId\|\|''\) \+ '\\'\\'',\\'\\'' \+ s\.cls \+ '\\'\\''\)" title="编辑此项">✎<\/button>/g,
    function(match) {
        stageCount++;
        return match
            .replace('editStage', 'openEditPartModal')
            .replace(',\\\'\\',\\\'\\' + s.cls + \\\'\\\'', '')  // remove s.cls param
            .replace('title="编辑此项"', 'title="编辑此钩子"');
    }
);
console.log('[3] Stage buttons replaced:', stageCount);

// ================================================================
// 4. Hook item button - regex
// ================================================================
let hookCount = 0;
content = content.replace(
    /onclick="editPart\(\\'\\'' \+ curCategory \+ '\\'\\'',\\'\\'' \+ hookItem\.id \+ '\\'\\''\)">✎ 编辑<\/button>/g,
    function(match) {
        hookCount++;
        return match.replace('editPart', 'openEditPartModal');
    }
);
console.log('[4] Hook buttons replaced:', hookCount);

// ================================================================
// 5. Related card buttons - regex  
// ================================================================
let relCount = 0;
content = content.replace(
    /editPart\(\\'\\'' \+ curCategory \+ '\\'\\'',\\'\\'' \+ rel\.id \+ '\\'\\''\)">✕<\/button>/g,
    function(match) {
        relCount++;
        return match.replace('editPart', 'openEditPartModal');
    }
);
console.log('[5] Related buttons replaced:', relCount);
...
const editStageCnt = (v.match(/onclick[^>]*editStage/g)||[]).length;
const editPartFunc = (v.match(/function editPart\(/g)||[]).length;
console.log('\nVerify:');
console.log('  openEditPartModal calls:', openCnt);
console.log('  editStage in onclick:', editStageCnt, '(should be 0)');
