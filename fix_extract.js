...
// 3. Stage card button - EXTRACT exact strings
...
if (si >= 0) {
    ...
    if (btnEnd >= 0) {
        ...
        // s.cls is at position 92 in the string
        const clsIdx = oldStage.indexOf('s.cls');
        // The ' before s.cls is the opening quote
        const beforeScls = clsIdx; // position of ' before s
        // The ' after " + s.cls + " closes the param
        // s.cls starts at 92, ends at 97 ('s' is 5 chars)
        // After s.cls there's: space(98) + +(99) + space(100) + '(101)
        // Then: \ at 102, ' at 103
        // closeQ needs to be at the ' that closes the s.cls param
        // After the param there's: \ at 102, ' at 103
        // The param closing ' is at position 103
        let closeQ = clsIdx + 5; // after 's' of s.cls
        while (closeQ < oldStage.length && oldStage[closeQ] !== "'") closeQ++;
        // Now closeQ is at the ' after + s.cls +
        // But the param closing ' is AFTER the \'
        // Actually: the s.cls param is: '\' + s.cls + '\'
        // After s.cls: space + + space + \ + '
        // closeQ is at the ' right after the \
        // That ' is the closing ' of the s.cls param
        // So closeQ+1 starts AFTER the closing '
        
        const beforeCls = oldStage.substring(0, clsIdx - 1); // back to the ,
        const afterClsPart = oldStage.substring(closeQ + 1);
...
console.log('  Stage btn has s.cls:', stageHasCls, '(should be false)');
